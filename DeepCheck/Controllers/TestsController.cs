...
public class TestsController : ControllerBase
{
    ...
    public TestsController(ILogger<TestsController> logger, IEnumerable<ITest> tests, ITestRunService service, IOptions<KumaServiceSettings> config)
    {
        _logger = logger;
        _tests = tests;
        _service = service;
    ...
    public async Task<ActionResult<TestRunInfo>> ExecuteTestByName([FromRoute] string testName, [FromServices] IHostEnvironment env,
        CancellationToken cancellationToken = default)
    {
        if (env.IsDevelopment())
        {
    ...
    public async Task<IActionResult> GetLastStep(string testName, string stepName, CancellationToken cancellationToken = default)
    {
        ...
        // if found but not ok
        if (response.Status != LastRunStatusEnum.Ok || (response.FinishedAt != default && timestamp - response.FinishedAt > TimeSpan.FromSeconds(this._kumaServiceSettings.OlderThanInSeconds)
            ))
        {
